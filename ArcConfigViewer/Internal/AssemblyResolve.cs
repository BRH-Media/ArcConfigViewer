...
namespace ArcConfigViewer.Internal
{
    ...
    {
        //THESE ARE THE FORMAL LOCATIONS OF ArcConfigViewer FILES.
        ...
        /// </summary>
        public static string XmlDirectory { get; } = @"xml"; //NOTE: These are generated by the compiler; not by ArcConfigViewer.

        /// <summary>
        /// Directory of the currently executing ArcConfigViewer.exe binary
        /// </summary>
        public static string ArcLocation { get; } = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule?.FileName ?? string.Empty);
        ...
        public static Assembly HandleResolve(object sender, ResolveEventArgs args)
        {
            ...
            // We can't handle missing libraries if there's a missing ArcConfigViewer path

            if (!string.IsNullOrEmpty(ArcLocation))
            {
            ...
            // Kill current ArcConfigViewer process
        ...
        public static void LogAssemblyResolve(string fileName, string occurrence)
        {
            try
            {
                if (!Environment.GetCommandLineArgs().Contains("-debug")) return;
